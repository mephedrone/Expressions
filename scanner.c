...
/*	Jurjen Kooistra s3015076 && Roberto Navarro s3141292 */
/*	FEB 20TH 2017 */
...
void evaluateExpression(List li)
{
  int multipleVariables=0;
  int equalSignCount=0;
  char *currentVariable = malloc(1000*sizeof(char));
  char *savedVariable = malloc(1000*sizeof(char));
  int entryCount=0;
  ...
  while (li != NULL) 
  {
    ...
    case Number: 
      if (startRecording==1)
        degree=(li->t).number, startRecording=0;
      break;
    
    case Identifier: 
      entryCount++;
     
      strcpy(currentVariable, (li->t).identifier);
      if (entryCount==1)
        strcpy(savedVariable, currentVariable);

      if (strcmp(currentVariable, savedVariable)!=0)
        multipleVariables=1;      
      break;
  ...
  else if ((multipleVariables && equalSignCount==1) || entryCount==0)
